...
local Library = {
    ...
    Scheme = {
        ...
        AccentColor = Color3.fromRGB(128, 92, 255),
        OutlineColor = Color3.fromRGB(44, 44, 52),
    ...
    function Funcs:AddValueDropdown(Idx, Info)
        ...
        local function UpdateCloseButtonStyle()
            if Info.Multi then
                ...
                CloseButtonStroke.Color = Library.Scheme.AccentColor:Lerp(Library.Scheme.WhiteColor, 0.2)
        ...
        local function GetPopupMetrics()
            ...
            local actionH = math.clamp(height * 0.05, 24, 28)
        ...
        local function ApplyPopupLayout()
            ...
            if Info.Multi then
                ...
                ClearButton.Size = UDim2.fromOffset(buttonW, M.actionH)
                ClearButton.Position = UDim2.new(1, -buttonW, 0, 0)
                SelectAllButton.Size = UDim2.fromOffset(buttonW, M.actionH)
                SelectAllButton.Position = UDim2.new(1, -(buttonW * 2 + buttonGap), 0, 0)
                rowTop = rowTop + M.actionH + math.max(6, math.floor(M.gap * 0.7))
            else
        ...
        local function ApplyBulkSelection(enableAll)
            if not Info.Multi then
                return
            end
            local nextValues = {}
        ...
        function Dropdown:Display()
            ...
            if Info.Multi then
                -- Always show the actual selected values instead of summaries such
                -- as "8 selected" or "+6 more". The TextButton already uses
                -- TextTruncate.AtEnd, so long selections stay clean in the UI.
                local Parts = {}

                for _, Value in pairs(Dropdown.Values) do
                    if Dropdown.Value[Value] then
                        table.insert(Parts, tostring(GetDisplayText(Value)))
                    end
                end
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        -- ZANJIHUB header uses independent positioning instead of a horizontal
        -- UIListLayout. This keeps the TITLE mathematically centered in the
        -- sidebar while the transparent logo sits in the unused left-side space.
    ...
    function Window:AddTab(...)
        ...
        function Tab:Hover(Hovering)
            ...
            TweenService:Create(TabButton, Library.TweenInfo, {
                BackgroundTransparency = Hovering and 0.64 or 0.78,
            }):Play()
            TweenService:Create(TabLabel, Library.TweenInfo, {
                TextTransparency = Hovering and 0.25 or 0.5,
            }):Play()
            TweenService:Create(TabButtonStroke, Library.TweenInfo, {
                Transparency = Hovering and 0.58 or 0.72,
            }):Play()
            if TabIcon then
    ...
    -- Z-key toggle removed; floating logo is the only UI toggle.
